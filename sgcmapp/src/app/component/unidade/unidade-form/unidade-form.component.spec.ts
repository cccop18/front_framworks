...
describe('UnidadeFormComponent', () => {
  ...
  const serviceMock = {
    consultarPorId: jasmine.createSpy('consultarPorId').and.returnValue(of(mockUnidades[0])),
    salvar: jasmine.createSpy('salvar').and.returnValue(of(mockUnidades[0]))
  };
  ...
  it('deve chamar servico.save quando clicar no botão "Salvar"', () => {
    ...
    botaoSalvar.click();
    expect(serviceMock.salvar).toHaveBeenCalledWith(mockUnidades.find(item => item.id == 1));
  });
